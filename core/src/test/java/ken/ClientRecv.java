...
public class ClientRecv extends Thread {
	...
	final static String router_host = "tcp://localhost:"
			+ EConfig.loadAll("easyEDA.yaml").get(
					EConfig.EDA_ROUTER_OUTGOING_PORT);
	String fkey;
	String tid;
	SocketID sid;
	...
	public void run() {
		int count = 0;
		String ask = (String) EConfig.loadAll("easyEDA.yaml").get(
				EConfig.EDA_ROUTER_HAND_ASK);
		String report = (String) EConfig.loadAll("easyEDA.yaml").get(
