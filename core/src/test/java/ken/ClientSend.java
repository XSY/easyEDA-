...
public class ClientSend extends Thread {
	...
	final static String router_host = "tcp://localhost:"
			+ EConfig.loadAll("easyEDA.yaml").get(
					EConfig.EDA_ROUTER_INCOMING_PORT);
	// final static String router_host =
	// "tcp://localhost:"+EConfig.loadAll("easyEDA.yaml").get(EConfig.EDA_PIVOT_INCOMING_PORT);
	...
	public void run() {
		...
		while (!Thread.currentThread().isInterrupted()) {
			...
			LOG.debug("recv ok");
			if (reply != null) {
				LOG.debug("[Client-" + _id + "] received reply:[" + reply + "]");
			}
