...
public class ZDispatcher {
	...
	public void registerHandler(ZMQ.Socket socket,
			ZMessageHandler messageHandler, ZSender sender) {
	...
	public void registerHandler(ZMQ.Socket socket,
		...
		SocketDispatcher socketDispatcher = new SocketDispatcher(socket,
				messageHandler, sender, threadpool);
		if (dispatchers.putIfAbsent(socket, socketDispatcher) != null) {
			throw new IllegalArgumentException(
					"This socket already have a message handler");
		}
	...
	public void unregisterHandler(ZMQ.Socket socket) {
		...
		if (removedDispatcher == null) {
			throw new IllegalArgumentException(
					"This socket doesn't have a message handler");
		}
	...
	private static final class SocketDispatcher implements Runnable {
		...
		public SocketDispatcher(ZMQ.Socket socket, ZMessageHandler handler,
				ZSender sender, ExecutorService handleThreadpool) {
			this.socket = socket;
		...
		private void doReceive() {
			...
			while (active && remainingBuffer-- > 0
					&& (msg = ZMsg.recvMsg(socket, ZMQ.DONTWAIT)) != null
					&& msg.size() > 0 && msg.getFirst().hasData()) {
				in.add(msg);
			}
		}
		...
		private void doHandle() {
			if (!in.isEmpty() && busy.compareAndSet(false, true)) {
				threadpool.submit(new Runnable() {
					@Override
					public void run() {
						// add this declaration, different from the initial
						// code. find the reason
						@SuppressWarnings("static-access")
						ZMessageBuffer messages = SocketDispatcher.this.messages
								.get();
						messages.drainFrom(in);
						busy.set(false);
						for (int i = 0; i <= messages.lastValidIndex; i++) {
							if (active) {
								handler.handleMessage(sender,
										messages.buffer[i]);
							}
						}
					}
