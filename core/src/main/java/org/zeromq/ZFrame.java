 ...
 *         frames across 0MQ sockets. A 'frame' corresponds to one underlying
 *         zmq_msg_t in the libzmq code. When you read a frame from a socket,
 *         the more() method indicates if the frame is part of an unfinished
 *         multipart message. The send() method normally destroys the frame, but
 *         with the ZFRAME_REUSE flag, you can send the same frame many times.
 *         Frames are binary, and this class has no special support for text
 *         data.
 * 
 *         Based on <a
 *         href="http://github.com/zeromq/czmq/blob/master/src/zframe.c"
 *         >zframe.c</a> in czmq
 * 
 */
public class ZFrame {
