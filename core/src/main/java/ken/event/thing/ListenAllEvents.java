...
public class ListenAllEvents {
	...
	public static void main(String[] args) throws Exception {
		...
		conf.setDebug(true);
		conf.putAll(EConfig.loadAll("easyEDA.yaml")); 
		// add easyEDA's configuration in. call for loadAll("filepath") instead of loadAll()

		String thing_n = (String) conf.get(EConfig.EDA_THING_LISTENALL_NAME);
		int thing_p = ((Integer) conf.get(EConfig.EDA_THING_LISTENALL_WORKER)).intValue();

		String a_n = (String) conf.get(EConfig.EDA_CH_MASTER_NAME);
		int a_p = ((Integer) conf.get(EConfig.EDA_CH_MASTER_PARALLEL))
				.intValue();

		String a1_n = (String) conf.get(EConfig.EDA_PROC_ARC_NAME);
		int a1_p = ((Integer) conf.get(EConfig.EDA_PROC_ARC_PARALLEL))
				.intValue();

		String a2_n = (String) conf.get(EConfig.EDA_PROC_ADDR_NAME);
		int a2_p = ((Integer) conf.get(EConfig.EDA_PROC_ADDR_PARALLEL))
				.intValue();

		String a21_n = (String) conf.get(EConfig.EDA_PROC_ROUT_NAME);
		int a21_p = ((Integer) conf.get(EConfig.EDA_PROC_ROUT_PARALLEL))
				.intValue();
		
		//conf.put("topology.ackers", 1);
