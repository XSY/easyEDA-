	...
	private void follow() throws IOException, ClassNotFoundException,
		...
		while (!Thread.currentThread().isInterrupted()) {
			...
			LOG.debug(Thread.currentThread().getName() + "[Worker]-[fkey:"
					+ _key + "|tid=" + sid.getThreadID() + "] is waiting to recv...");

			poller.register(socket, Poller.POLLIN);
			poller.poll(8000 * 1000);
			if (poller.pollin(0)) {
				// received
				
				String client_addr = new String(socket.recv(0));
				if (ask.equals(client_addr)) { //when router will send ask first??
					...
					try {
						...
						LOG.debug(Thread.currentThread().getName()
								+ "[Worker]-[fkey:" + _key + "|tid=" + 1
								+ "] since follower started, received ["
								+ count + "] events!");
					} catch (IOException e) {
						LOG.error(e);
